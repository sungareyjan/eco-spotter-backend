...
describe('User CRUD with Controllers', () => {
    it('should create a user', async () => {
        ...
        console.log(res.body); // check the actual response

        expect(res.statusCode).toBe(201);
        expect(res.body.data.user.username).toBe('juancruz');
        expect(res.body.data.user.email).toBe('juan.cruz@example.com');

        expect(res.body.data.auth).toBeDefined();
        expect(res.body.data.auth.accessToken).toBeDefined();
        expect(res.body.data.auth.refreshToken).toBeDefined();
